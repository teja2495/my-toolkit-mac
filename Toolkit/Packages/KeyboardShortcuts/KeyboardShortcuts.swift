...
public enum KeyboardShortcuts {
	...
	*/
	...
	static var hotKeys = [Shortcut: HotKey]()
	...
	static var streamKeyDownHandlers = [Name: [UUID: () -> Void]]()
	static var streamKeyUpHandlers = [Name: [UUID: () -> Void]]()
	static var streamShortcutKeyDownHandlers = [Shortcut: [UUID: () -> Void]]()
	static var streamShortcutKeyUpHandlers = [Shortcut: [UUID: () -> Void]]()
	...
	*/
	static func removeStreamHandlerEntry<Key: Hashable>(
		_ id: UUID,
		for key: Key,
	...
	static func registerIfNeeded(for shortcut: Shortcut) {
	...
	*/
	static func registerIfNeeded(for name: Name) {
		guard hasActiveHandlers(for: name) else {
			return
		}
	...
	*/
	static func unregisterIfNeeded(for name: Name, excludingCurrentName: Bool = true) {
		guard let shortcut = getShortcut(for: name) else {
			return
		}
	...
	static func unregisterIfNeeded(for shortcut: Shortcut) {
